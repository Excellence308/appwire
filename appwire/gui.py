...
class Window(Gtk.Window):
    def __init__(self):
        ...
        for label, action in [('Import…', self.import_config), ('Start', lambda *_: self.action('start')), ('Stop', lambda *_: self.action('stop')), ('Refresh', lambda *_: self.refresh()), ('Check exit IP', self.probe)]:
            button = Gtk.Button(label=label)
            button.connect('clicked', action)
            row.pack_start(button, False, False, 0)
        self.details = Gtk.TextView(editable=False, monospace=True, wrap_mode=Gtk.WrapMode.WORD_CHAR)
        scroll = Gtk.ScrolledWindow()
        scroll.set_min_content_height(200)
        scroll.add(self.details)
        box.pack_start(scroll, True, True, 0)
        self.command = Gtk.Entry(placeholder_text='Application and arguments, for example: faugus-launcher')
        ...
        self.message = Gtk.Label(label='Start a profile before launching. Stop disconnects apps still using it.', xalign=0, wrap=True)
    ...
    def show_status(self, value):
        ...
        for peer in value.get('peers', []):
            ...
            lines.extend(['', 'Endpoint: ' + peer['endpoint'], 'Handshake: ' + (f'{age} seconds ago' if age is not None else 'waiting for first handshake'), f"Received: {peer['received'] / 1048576:.2f} MiB    Sent: {peer['sent'] / 1048576:.2f} MiB"])
        apps = value.get('applications', [])
        if apps:
            lines.extend(['', 'Applications:'] + [f"  {a['name']}  (PID {a['pid']})" for a in apps])
    ...
    def refresh(self):
        def done(value):
            ...
            if names:
                ...
                self.message.set_text('Import a standard WireGuard configuration to begin.')
    ...
    def probe(self, *_):
        name = self.selected()
        if name:
            self.work(lambda: client.probe(name), lambda ip: self.message.set_text('Verified IPv4 exit: ' + ip))
    ...
    def launch(self, *_):
        ...
        try:
            ...
            if name and argv:
                ...
                directory = Path(os.environ.get('XDG_STATE_HOME', str(Path.home() / '.local/state'))) / 'appwire'
                directory.mkdir(mode=0o700, parents=True, exist_ok=True)
                log = directory / 'applications.log'
                fd = os.open(log, os.O_WRONLY | os.O_CREAT | os.O_APPEND, 0o600)
                try:
                    process = subprocess.Popen(['/usr/bin/appwire', 'run', name, '--', *argv], stdin=subprocess.DEVNULL, stdout=fd, stderr=fd, start_new_session=True)
                finally:
                    os.close(fd)
                ...
                def poll():
                    if process.poll() is None:
                        return True
                    self.message.set_text(f'Application exited ({process.returncode}). Log: {log}')
                    return False
    ...
    def import_config(self, *_):
        dialog = Gtk.FileChooserDialog(title='Import WireGuard config', transient_for=self, action=Gtk.FileChooserAction.OPEN)
        dialog.add_buttons('Cancel', Gtk.ResponseType.CANCEL, 'Open', Gtk.ResponseType.OK)
        if dialog.run() == Gtk.ResponseType.OK:
            filename = dialog.get_filename()
            dialog.destroy()
            name_dialog = Gtk.Dialog(title='Profile name', transient_for=self, modal=True)
            name_dialog.add_buttons('Cancel', Gtk.ResponseType.CANCEL, 'Import', Gtk.ResponseType.OK)
